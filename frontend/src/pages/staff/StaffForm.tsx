...
export default function StaffForm() {
  ...
  const majors = useCatalogueStore((s) => s.majors);
  const loadMajors = useCatalogueStore((s) => s.loadMajors);
  ...
  }, [loadDepartments, loadMajors, loadHealthCenters]);
    ...
    async function loadUser() {
      try {
        ...
        setForm((previous) => ({
          ...
          majorId: String(user.healthcareWorker?.majorId ?? ''),
          healthCenterId: user.healthcareWorker?.healthCenterId ?? '',
        }));
  ...
  return (
    <div className='mx-auto max-w-2xl'>
      <h1 className='mb-4 text-2xl font-bold text-slate-900'>
        {isEditing ? 'Editar personal de salud' : 'Nuevo personal de salud'}
      </h1>
      <Card>
